...
public class ArmorScarabLayer<T extends Player,M extends ScarabModel<T>,A extends ScarabModel<T>> extends RenderLayer<T,M> {
    private static final Map<String, ResourceLocation> ARMOR_LOCATION_CACHE = Maps.newHashMap();
    private final A outerModel;
    private final TextureAtlas armorTrimAtlas;

    public ArmorScarabLayer(RenderLayerParent<T, M> renderer, ModelManager modelManager) {
        super(renderer);
        this.outerModel = (A) new ScarabModel<>(Minecraft.getInstance().getEntityModels().bakeLayer(ScarabModel.ARMOR_LOCATION));
    ...
    protected void setPartVisibility(A model, EquipmentSlot slot) {
        model.root().getAllParts().forEach(e->{
            e.visible = false;
        });
        switch (slot) {
            case HEAD:
